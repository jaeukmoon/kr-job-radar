...
| LG그룹 (계열사 포함) | api.careers.lg.com retrieveJobNoticesList (POST) | 전체 중 AI/ML/데이터 (LG전자·유플러스·CNS·디스플레이 등) |
| KT (계열사 포함) | recruit.kt.com /api/recruit (GET) | 전체 중 AI/ML/데이터 (KT·BC카드·kt ds 등) |
...
```
fetch_jobs.py       # stdlib-only 정적 fetcher (8소스 + AI/ML 필터 + first_seen 원장 + 브라우저 병합)
